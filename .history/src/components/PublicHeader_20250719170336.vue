...
const navigation = ref<{ name: string; path: string }[]>([]) // from API

const fetchNavigation = async () => {
  try {
    loading.value = true
    const res = await axiosApi.post('/webtour/data/navigation/list')
    const rowData = res.data.rows
    navigation.value = rowData.filters((item: any) => item.is_active === 1).sort((a: any, b: any) => a.order_index - b.order_index)
    console.log('Navigation fetched:', navigation.value)
  } catch (error) {
